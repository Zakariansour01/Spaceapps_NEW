...
const AU_UNITS = 100;
const LENGTH_SCALE = AU_UNITS / AU_KM;
const SIZE_MULTIPLIER = 50;
let TIME_SCALE = 5000000;
...
const earthBase = {
    ...
    M_deg: -2.47311027,
    mean_motion_deg_day: 35999.04917617 / 36525
};
...
const moonTexture = textureLoader.load('Textures/mars.jpg');
const moonMaterial = new THREE.MeshBasicMaterial({ color: 0xff0000 });
...
document.getElementById('compute').addEventListener('click', () => {
    ...
    const omega_deg = varpi_deg_input - Omega_deg_input;
...
camera.position.z = 200 * SIZE_MULTIPLIER;
...
function createAsteroidWithRealOrbit(name, orbitClass, diameter, orbitalData) {
    ...
    const a = a_AU * AU_UNITS * SIZE_MULTIPLIER;
...
function getAsteroidPosition(a, e, i, omega, Omega, M) {
    ...
    const { xprime, yprime } = orbitalPlaneCoords(a / (AU_UNITS * SIZE_MULTIPLIER), e, E);
...
function updateAsteroidInfo() {
    ...
    const radiusMultiplier = selectedAsteroid.radiusMultiplier || 1;
    ...
    if (whatIfButton) {
        whatIfButton.addEventListener('click', () => {
            if (selectedAsteroid) {
                ...
                selectedAsteroidRadiusName.textContent = `Asteroid: ${selectedAsteroid.name}`;
    ...
    if (distance < collisionThreshold) {
        if (!asteroid.hasCollided) {
            ...
            if (!hasRedirected) {
                ...
                const realRadiusMeters = asteroid.diameter / 2;
                
                const data = {
                    name: asteroid.name,
                    speed: asteroid.current_v_peri.toFixed(2),
...
const distance = (EARTH_RADIUS_KM * LENGTH_SCALE * SIZE_MULTIPLIER * 500) * 0.5;
...
function updateMoon(delta) {
    ...
    const moon_orbit_radius = EARTH_MOON_DISTANCE_KM * LENGTH_SCALE * SIZE_MULTIPLIER* 20;
